...
public class BW1058TabSorter extends JavaPlugin {
    ...
    public void onEnable(){
        ...
        if(configManager.getConfig().getBoolean("header-footer.enabled")){
            hTask = new HeaderFooterRefreshTask(configManager.getConfig().getLong("header-footer.refresh-interval")*20L);
            hTask.startTask();
        }
    ...
    public void sendHeaderFooter(Player p){
        if(!configManager.getConfig().getBoolean("header-footer.enabled"))return;
